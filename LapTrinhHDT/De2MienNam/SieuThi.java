...
public class SieuThi {
    ...
    public void suaInForSanPham() {
        System.out.printf("Nhap ma San Pham muon sua: ");
        int maSP = sc.nextInt();
        sc.nextLine();
        SanPham sp = checkMaSanPham(maSP);
        if(sp == null) {
            System.out.println("-> Khong co Ma SP nay!!");
            return;
        }
        System.out.printf("Nhap ten San Pham: ");
        String tenSP = sc.nextLine();
    ...
    public void inKhachHangMuaNhieuNhat() {
        float max = listKhachHang.get(0).tongTienMuaHang;
        int index = 0;
        for(int i = 0; i < listKhachHang.size(); i++) {
            if(listKhachHang.get(i).tongTienMuaHang > max) {
                max = listKhachHang.get(i).tongTienMuaHang;
                index = i;
            }
        }
        System.out.printf("Khach hang mua nhieu nhat la: %s, TongTien: %.1f", listKhachHang.get(index).tenKH, listKhachHang.get(index).tongTienMuaHang);
        System.out.println("");
    }   
}
